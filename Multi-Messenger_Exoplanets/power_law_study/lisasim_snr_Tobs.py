...
File: lisasim_snr_Tobs.py

Purpose: Run the LISA simulations used to test the SNR residual power law dependence on the observational period.
...
Output:
- text file containing the observation time, the total SNR (with and without the exoplanet), and the SNR residual 
...
# range of observation times for the simulation
observation_times = np.logspace(np.log10(0.5), np.log10(10), 10)*u.year.to(u.s)
...
if os.path.exists('../LISAsim_data_files/snrdiff_Tobs.txt'):
    os.remove('../LISAsim_data_files/snrdiff_Tobs.txt')
...
for Tobs in observation_times:
    ...
    # -------------- PSDs ----------------
    PSD_chirp_noexo = PSD_1s(chirp_noexo_DFT, dt, Nt, norm).real
    PSD_chirp       = PSD_1s(chirp_DFT, dt, Nt, norm).real
    
    PSD_chirp_diff = PSD_1s(chirp_diff_DFT, dt, Nt, norm).real
    
    
    
    # -------------- Total SNR (without / with the exoplanet) ----------------   
    SNRnoexo = np.sqrt( 2 * np.sum(PSD_chirp_noexo / S_X20_EQarm(freqs, Larm)) )
    SNRexo   = np.sqrt( 2 * np.sum(PSD_chirp / S_X20_EQarm(freqs, Larm)) )
    ...
    # -------------------------------------
    # Save the output  
    f = open('../LISAsim_data_files/snrdiff_Tobs.txt', 'a')
    np.savetxt(f, np.array([[Tobs],[SNRexo],[SNRnoexo],[SNRdiff]]).T)
    f.close()
